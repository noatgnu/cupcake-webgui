...
export class ReagentMetadataModal implements OnInit {
  ...
  officialColumns: SdrfColumnConfig[] = this.getReagentRelatedColumns();
  ...
  private getReagentRelatedColumns(): SdrfColumnConfig[] {
    return OFFICIAL_SDRF_COLUMNS;
  }

  ngOnInit(): void {
    if (this.storedReagentId) {
      this.loadMetadata();
    }
    this.loadTemplates();
  }
  ...
  loadMetadata(): void {
    if (!this.storedReagentId) return;

    this.loading.set(true);
    this.reagentService.getStoredReagentMetadata(this.storedReagentId).subscribe({
      next: (metadataTable) => {
  ...
  addField(): void {
    ...
    if (!this.storedReagentId || !this.metadataTableId) {
      this.toastService.error('No reagent or metadata table associated');
      return;
    }
    ...
    this.reagentService.addStoredReagentMetadataColumn(this.storedReagentId, {
  ...
  saveFieldValue(field: MetadataColumn, newValue: string): void {
    if (!field.id || !this.storedReagentId) return;

    this.saving.set(true);
    this.reagentService.updateStoredReagentMetadataValue(this.storedReagentId, {
      columnId: field.id,
      value: newValue
    }).subscribe({
      next: (response) => {
  ...
  deleteField(field: MetadataColumn): void {
    ...
    this.reagentService.removeStoredReagentMetadataColumn(this.storedReagentId, field.id.toString()).subscribe({
      next: () => {
